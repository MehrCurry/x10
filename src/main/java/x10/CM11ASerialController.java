...
/**
 * CM11ASerialController is an X10 Controller that bridges x10 hardware and
 * software by communicating via a SerialPort with the x10 "CM11A" module. <BR>
 * <BR>
 * 
...
public class CM11ASerialController implements Runnable, Controller {
	private static final Logger logger=LoggerFactory.getLogger(CM11ASerialController.class);
	/**
	 ...
	 * specified comport. On a Windows based PC, the comport is of the form
	 * "COM1".
	 * 
	 * @param comport
	 *            the communications port in which the "CM11A" module is
	 *            connected.
	...
	/**
	 * shutdown tells the controller to finish all commands in the queue and
	 * then gracefully disconnects the serial port connection.
	 * 
	 * @param millis
	 *            the number of milliseconds to wait for a graceful shutdown.
	 * @exception OperationTimedOutException
	 *                thrown if the Controller has not completely shutdown in
	...
	public void shutdown(long millis) throws OperationTimedOutException,
			...
			try {
				...
				if (running) {
					throw new OperationTimedOutException(
							"Timed out while waiting for CM11ASerialController to shutdown");
				}
	...
	/**
	 * shutdownNow shuts down the controller and closes the serial port
		...
		if (nextCommand != null) {
			if (checksum == getChecksum(nextCommand.getAddress())) {
					...
					if (nextCommand == STOP) {
						...
						if (ready == READY) {
							...
							if (((byte) fromX10.readByte()) == getChecksum(nextCommand
								...
								if (ready == READY) {
									dispatcher.dispatchUnitEvent(new UnitEvent(
				...
				LogHandler.logMessage(
						"CheckSum: " + Integer.toHexString(checksum), 2);
			}
		}
				...
				if (isAddr[i]) {
					for (int j = i + 1; j < isAddr.length; j++) {
						if (!isAddr[j]) {
							...
							byte level = 0;
							if ((function == Command.DIM)
									|| (function == Command.BRIGHT)) {
								level = data[j + 1];
							}
					...
					case Command.ALL_LIGHTS_OFF:
						dispatcher.dispatchUnitEvent(new UnitEvent(new Command(
								function, function, (byte) 0)));
						break;
					case Command.DIM:
					case Command.BRIGHT:
						i++;
					...
					case Command.OFF:
						for (int l = 0; l < lastAddresses.length; l++) {
							dispatcher.dispatchUnitEvent(new UnitEvent(
									new Command(lastAddresses[l], function,
											(byte) level)));
						}
						break;
					}
