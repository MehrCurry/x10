...
/**
 * CM17ASerialController is an X10 Controller that bridges x10 hardware and
 * software by communicating via a SerialPort with the x10 "Firecracker" module. <BR>
 * <BR>
 * 
...
public class CM17ASerialController implements Runnable, Controller {
	 ...
	 * 
	 * @param comport
	 *            the communications port in which the "Firecracker" module is
	 *            connected.
	...
	public CM17ASerialController(String comport) throws IOException {
		...
		InputStream commandStream = loader
				.getResourceAsStream(FIRECRACKER_MAP_FILE);
		...
		while (running) {
			try {
				...
				if (nextCommand == STOP) {
					...
					switch (function) {
					case Command.ON:
						toFirecracker((String) commandTable.get(houseCode + ""
								+ unitCode + "ON"));
						break;
					case Command.OFF:
						toFirecracker((String) commandTable.get(houseCode + ""
	...
	private void resetFirecracker() {
		throw new UnsupportedOperationException("Not yet ported to RXTX");
		// sp.setDTR(false);
		// sp.setRTS(false);
